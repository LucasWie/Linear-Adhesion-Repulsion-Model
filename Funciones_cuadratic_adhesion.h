...
void calcular_vel(vector<Particula> &particulas, vector < vector<int> > vecinos, double r_eq, double r_cut, double f_rep, double f_adh, default_random_engine &generator,float eta){
  ...
  for (int i = 0; i<particulas.size(); i++){
    ...
    for (int id_vec = 0; id_vec < vecinos[i].size(); id_vec++){
      ...
      if(dist < r_cut) {
        if (dist < r_eq){   // Está demasiado cerca, habrá fuerza repulsiva
        ...
        else{ // La Distancia es intermedia
          int e_i = (particulas[vecinos[i][id_vec]].get_pos_x()-particulas[i].get_pos_x())/dist;//esto deberìa ser 1 o -1
          vec_forces = vec_forces + e_i* (-f_adh*((dist-r_eq)*(dist-r_cut) ) );//Hacer cosas de fuerza nueva cuadrática.
        }
      }
...
//recibe una distribucion de velocidades, y devuelve un número aleatoreo tomado de esa distribucion
